...
//! higher-precision numbers; they all work with `f64`s and return tuples of `f64`s, which are then
//! used in higher-precision operations.
//!
//! This module is aware of the `no_fma` feature. If it's enabled, further constants and functions
//! are used to split floats into high and low words, necessary for the multiplication algorithm.
//! If the feature is not enabled, multiplication instead depends on `f64`'s `mul_add` function,
//! which uses FMA under the hood if it's available.
