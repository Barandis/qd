...
//! more is needed.
//!
//! The choices are limited for higher-precision floating-point numbers. One choice is to use
//! 128-bit floating-point numbers, but Rust (and most languages outside Fortran and a few C++
//! flavors) doesn't have them.
//!
//! A second choice is an arbitary-precision library. These are fantastic in that they can do
...
//! they have to essentially re-implement math for that internal representation. This is slow.
//!
//! Fortunately, while a lot of applications need more than the language-provided precision, they
...
//! numbers are what this library provides.
//!
...
//!
//! The `Double` type has 106 bits of significand, meaning about 31 decimal digits, while the `Quad`
//! type has 212 bits (about 63 decimal digits). However, the exponents remain the same as in `f64`,
//! so the range of each type is similar to `f64` (max value of around ~10<sup>308</sup>). These
...
//! more digits than can be accurately represented by the type, the extra digits will be ignored).
//!
//! Once you have a double-double or a quad-double, you can use it just like you would an `f64`: all
//! of the mathematical operators work on them, the vast majority of methods work, etc. (see the
//! rest of this documentation for the full API). Each type has a full `Display` implementation,
//! meaning that you can use formatting strings with `format!`, `println!` and the like with all of
//! the formatting options that are available with `f64`.
//!
//! It's important to note that double-doubles and quad-doubles are incompatible with each other and
//! with other numbers, short of options to directly convert one to another. In other words, you
//! can't add an `f64` to a `Double` (though you can convert the `f64` to a `Double` and then add
//! them), and you can't multiply a `Quad` by an `i32` (though once again, you can convert the `i32`
//! to a `Quad` and then do it). This is typical of type casting in Rust (you also can't add an
