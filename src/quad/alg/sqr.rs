...
impl Quad {
    ...
    pub fn sqr(self) -> Quad {
        if self.is_infinite() {
            ...
            let (h0, l0) = basic::two_sqr(self.0);
            let (h1, l1) = basic::two_prod(2.0 * self.0, self.1);
            let (h2, l2) = basic::two_prod(2.0 * self.0, self.2);
            let (h3, l3) = basic::two_sqr(self.1);
            ...
            let (r1, a1) = basic::two_sum(h1, l0);

            let (b0, b1) = basic::two_sum(a1, l1);
            let (c0, c1) = basic::two_sum(h2, h3);
            let (d0, d1) = basic::two_sum(b0, c0);
            let (e0, e1) = basic::two_sum(b1, c1);
            let (f0, f1) = basic::two_sum(d1, e0);
            let (i0, i1) = basic::quick_two_sum(f0, e1 + f1);
            let (r2, j1) = basic::quick_two_sum(d0, i0);

            let (k0, k1) = basic::quick_two_sum(i1, j1);
            let (m0, m1) = basic::two_sum(h4, h5);
            let (n0, n1) = basic::two_sum(l2, l3);
            let (o0, o1) = basic::two_sum(m0, n0);
            let (r3, q1) = basic::two_sum(k0, o0);

            let r4 = m1 + n1 + o1 + k1 + q1;

            Quad::from(basic::renorm5(r0, r1, r2, r3, r4))
        }
    }
}
