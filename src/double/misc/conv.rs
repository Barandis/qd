...
impl Double {
    ...
    ///
    /// While it is possible for a `Double` to be created from a `u64`, whether
    /// or not the original is signed is not recorded (since `Double` itself is
    /// signed). The return value of this function can be cast to `u64` if
    /// necessary.
    #[inline]
    ...
    /// represent the double-double internally.
    #[inline]
    pub fn as_tuple(self) -> (f64, f64) {
        (self.0, self.1)
    }

    /// Assigns the components of a tuple to the components of the
    /// double-double.
    #[inline]
