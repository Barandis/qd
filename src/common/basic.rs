...
//! This module implements all of the low-level "primitives" used in the
//! algorithms of both double-doubles and quad-doubles. None of the functions
//! presented here are aware of the higher-precision numbers; they all work with
//! `f64`s and return tuples of `f64`s, which are then used in higher-precision
//! operations.
//!
//! This module is aware of the `no_fma` feature. If it's enabled, further
//! constants and functions are used to split floats into high and low words,
//! necessary for the multiplication algorithm. If the feature is not enabled,
...
/// components.
///
/// It essentially acts as a mask to isolate one half of the mantissa from the
/// other, and is therefore chosen to be a point halfway into the mantissa's bit
/// field.
///
...
///
/// If the absolute value of the value being split exceeds this, the number is
/// reduced before processing and then increased by the same factor after
/// processing. This increases accuracy in very large (and small) numbers where
/// accuracy is naturally lessened in floating-point numbers.
///
/// This value is 2<sup>996</sup>, which is the highest power of two that is
/// less than 10<sup>300</sup>.
#[cfg(no_fma)]
...
///
/// This value is 2<sup>28</sup>, or the inverse of the value used before
/// splitting.
#[cfg(no_fma)]
...
///
/// This calculation performs 3 floating-point operations. This is more
/// efficient than [`two_sum`](#fn.two_sum) but carries the restriction that |a|
/// >= |b|, which [`two_sum`](#fn.two_sum) does not.
#[inline]
...
/// Calculates fl(a + b) and err(a + b).
///
/// This calculation performs 6 floating-point operations. It is less efficient
/// than [`quick_two_sum`](#fn.quick_two_sum) but it carries no restrictions on
/// its input values.
#[inline]
...
///
/// This is a helper function for use in multiplication functions when FMA is
/// not enabled (i.e., when the `no_fma` feature is enabled). It performs 4
/// floating-point operations, unless the number's absolute value is greater
/// than [`SPLIT_THRESHOLD`](constant.SPLIT_THRESHOLD.html). In this case it
/// performs 7 floating-point operations to increase precision in the large
/// number.
#[cfg(no_fma)]
#[inline]
...
/// Calculates fl(a * b) and err(a * b).
///
/// This implementation uses FMA and requires 2 floating-point operations
/// because of it. If FMA is not available but the `no_fma` feature is not
...
/// floating-point operations. If the arguments are particularly large, it can
/// require as many as 23 floating-point operations.
#[cfg(no_fma)]
#[inline]
pub fn two_prod(a: f64, b: f64) -> (f64, f64) {
    let p = a * b;
...
///
/// This implementation uses FMA and therefore requires 2 floating-point
/// instructions, the same as for multiplication. If FMA is not available but
/// the `no_fma` feature is not enabled, it will use considerably more
/// operations.
#[cfg(not(no_fma))]
#[inline]
...
/// Calculates fl(a * a) and err(a * a).
///
/// This implementation does not use FMA but takes advantage of optimizations
/// that can be made in multiplication due to the multiplied numbers being the
/// same. It therefore only uses a minimum of 12 floating-point operations,
/// though with a very large argument it can be 15.
#[cfg(no_fma)]
#[inline]
pub fn two_sqr(a: f64) -> (f64, f64) {
    let p = a * a;
